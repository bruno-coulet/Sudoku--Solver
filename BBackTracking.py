...
class Random(BGrid):
    ...
    def modify_first_0(self):
        ...
        # liste de tuples (ligne, colonne) contenant les coordonnées des zéros
        zeros = [(i, j) for i in range(9) for j in range(9) if lines[i][j] == "0"]
        
        # Remplace le premier zéro rencontré par un chiffre aléatoire entre 1 et 9 inclus
        for row, col in zeros:
            lines[row] = lines[row][:col] + str(random.randint(1, 9)) + lines[row][col+1:]
            break  # Sort de la boucle après avoir modifié le premier zéro
...
def main():
    ...
    while True:
        ...
        random_grid_str = generator.modify_first_0()
